...
var log = console;
/**
 * @file Library for fast creation network interfaces or API via json/bson
...
var myapi1 = reqs.new({
    ...
    , CPI: [ // Server programming interface: local user methods for other side
        'f3'
        , 'f4'
        , 'f5'
    ]
 ...
 */
function reqsParse(request, conn){ // Разбор входящего запроса
    var obj
        , fName
        , spi = this.SPI
        , params
 ...
 */
function reqsNew(options){ // Create new interface
    var localReqs = {
            SPI: options.SPI || {}
    ...
    // if( options.CPI === undefined ){ options.CPI = [] }
    ...
    // Binding `this` to user functions
    // for(k in rf){
    //         setBinds(this, rf, rf[k], k)
    // }
}
...
function makeApi(conn, cb){
    ...
    function createApi(methods, events){
        console.log('createApi', methods, events);
        if( isFunction( cb ) ){
            cb( reqsClient(lreqs, methods, lreqs.send) )
        } else {
...
// var basalt = require('basalt');
// var b = require("bson");
// var bson = b.BSONPure.BSON;

// var buffalo = require("buffalo");

// var c = require('./node/wss/config.json');
// var n = 100000;
...
// bt(basalt.encode, basalt.decode);
// bt(bson.serialize, bson.deserialize);
// bt(buffalo.serialize, buffalo.parse);
// bt(JSON.stringify, JSON.parse);

//

// exeF 2depth
// if( this.path.hasOwnProperty(requestPath) ){ // Есть такая команда?
//     f = this.path[requestPath];
//     if( typeof f === 'function' ){ // Это функция?
//         f(obj, conn);
//     } else { // Это не функция, значит это объект с командами для данного пути
//         for(key in obj){
...
//                 }
//             } else { // 404
//                 log.warn('reqs.jparse() 404 unknown command for this path: '+requestPath+' -> '+key);
//             }
//         }
//     }
// } else { // 404
//     log.warn('reqs.jparse() 404 unknown path: '+requestPath);
// }


// function setBinds(r, cmd, obj, name, conn, prName){
//     var f, key;
//     if( cmd.hasOwnProperty(name) ){
//         f = cmd[name];
//         if( typeof f === 'function' ){ // This is function?
//             cmd[name] = f.bind(r);
//             log.info('binded: '+name)
//         } else {  // Recurse down to next
//             for(key in obj){
//                 setBinds(r, f, obj[key], key, conn, name)
//             }
//         }
//     } else {
//         if( name === undefined ) { //
//             for(key in obj){
//                 setBinds(r, cmd, obj[key], key, conn, name)
//             }
//         } else {
//             if( prName === undefined ) {
//                 prName = ''
...
//                 prName = prName+' -> '
//             }
//             log.warn('reqs.parse().setBinds() 404 unknown command: '+prName+name)
//             log.warn(conn)
//         }
//     }
// }
